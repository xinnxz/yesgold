...
class YesCoinBot {
    ...
    async useSpecialBox(token) {
        ...
        try {
            ...
            if (response.data.code === 0) {
                await this.log('Mengaktifkan kotak spesial...', 'success');
                return true;
            } else {
                await this.log('Gagal mengaktifkan kotak spesial!', 'error');
                return false;
            }
        } catch (error) {
            return false;
        ...
        try {
            ...
            if (response.data.code === 0) {
                if (response.data.data.collectStatus) {
                    await this.log(`Buka kotak dapat ${response.data.data.collectAmount} Koin`, 'success');
                    return { success: true, collectedAmount: response.data.data.collectAmount };
                } else {
                    await this.log('Tidak ada kotak!', 'warning');
    ...
    async attemptCollectSpecialBox(token, boxType, initialCoinCount) {
        ...
        await this.log('Tidak bisa mengumpulkan kotak!', 'error');
        return 0;
    }
    ...
    async recoverCoinPool(token) {
        ...
        try {
            const response = await axios.post(url, {}, { headers });
            if (response.data.code === 0) {
                await this.log('Recovery berhasil!', 'success');
                return true;
            } else {
                await this.log('Recovery gagal!', 'error');
                return false;
            }
        } catch (error) {
            return false;
    ...
    async getTaskList(token) {
        ...
        try {
            ...
            if (response.data.code === 0) {
                ...
                await this.log(`Tidak bisa mendapatkan daftar tugas: ${response.data.message}`, 'error');
                return null;
            }
        } catch (error) {
    ...
    async finishTask(token, taskId) {
        ...
        try {
            ...
            if (response.data.code === 0) {
                await this.log(`Tugas ${taskId} selesai | Reward: ${response.data.data.bonusAmount}`, 'success');
                return true;
            } else {
                await this.log(`Tugas ${taskId} gagal: ${response.data.message}`, 'error');
                return false;
            }
        } catch (error) {
            await this.log(`Error saat menyelesaikan tugas: ${error.message}`, 'error');
            return false;
        }
    }
        ...
        if (tasks) {
            for (const task of tasks) {
                if (task.taskStatus === 0) {
                    ...
                    await this.log('Tugas sudah selesai', 'info');
                }
            }
        }
    }
    ...
    async upgradeLevel(token, currentLevel, targetLevel, upgradeType) {
        ...
        while (currentLevel < targetLevel) {
            try {
                ...
                if (response.data.code === 0) {
                    currentLevel++;
                    await this.log(`Upgrade ${upgradeTypeName} ke Lv ${currentLevel}`, 'success');
                } else {
                    await this.log(`Upgrade gagal: ${response.data.message}`, 'error');
                    break;
                }
            } catch (error) {
                await this.log('Error saat upgrade: ' + error.message, 'error');
                break;
            }
        }

        if (currentLevel === targetLevel) {
            await this.log(`${upgradeTypeName} sudah di level ${currentLevel}`, 'info');
        }
    }

    async wait(seconds) {
        for (let i = seconds; i > 0; i--) {
            process.stdout.write(`\r${colors.cyan(`[*] Tunggu ${Math.floor(i / 60)} menit ${i % 60} detik untuk lanjut`)}`.padEnd(80));
    ...
    quest() {
        ...
        return new Promise((resolve) => {
            readline.question("Mau ngerjain tugas? (y/n, default n): ", (taskAnswer) => {
                this.cekTaskEnable = taskAnswer.toLowerCase() === 'y' ? 'y' : 'n';

                readline.question("Mau upgrade multi? (y/n, default n): ", (multiAnswer) => {
                    this.upgradeMultiEnable = multiAnswer.toLowerCase() === 'y' ? 'y' : 'n';

                    readline.question("Mau upgrade fill rate? (y/n, default n): ", (fillAnswer) => {
                        this.upgradeFillEnable = fillAnswer.toLowerCase() === 'y' ? 'y' : 'n';

                        if (this.upgradeMultiEnable === 'y' || this.upgradeFillEnable === 'y') {
                            readline.question("Masukkan level maksimum untuk upgrade (default: 5): ", (maxLevelAnswer) => {
            ...
            if (accountBuildInfo.code === 0) {
                ...
                if (swipeBotLevel < 1) {
                    ...
                    if (upgradeResponse.data.code === 0) {
                        await this.log('Berhasil membeli SwipeBot', 'success');
                    } else {
                        await this.log('Gagal membeli SwipeBot', 'error');
                    }
                }
                if (swipeBotLevel >= 1 && !openSwipeBot) {
                    ...
                    if (toggleResponse.data.code === 0) {
                        await this.log('Berhasil mengaktifkan SwipeBot', 'success');
                    } else {
                        await this.log('Gagal mengaktifkan SwipeBot', 'error');
                    }
                }
                if (swipeBotLevel >= 1 && openSwipeBot) {
                    ...
                    if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
                        ...
                        if (claimResponse.data.code === 0) {
                            await this.log(`Berhasil klaim bonus offline, dapat ${claimResponse.data.data.collectAmount} koin`, 'success');
                        } else {
                            await this.log('Gagal klaim bonus offline', 'error');
                        }
                    }
                }
            } else {
                await this.log('Tidak bisa mengambil info SwipeBot', 'error');
            }
        } catch (error) {
            await this.log(`Error saat mengatur SwipeBot: ${error.message}`, 'error');
        }
    }
        ...
        while (true) {
            for (let i = 0; i < this.accounts.length; i++) {
                ...
                try {
                    ...
                    await this.log(`Gagal mendapatkan token untuk akun ${accountIndex}: ${error.message}`, 'error');
                    continue;
                }
                await this.randomDelay();
                const nickname = await this.getuser(token);
                await this.log(`========== Akun ${accountIndex} | ${nickname} ==========`, 'info');
                ...
                if (squadInfo && squadInfo.data.isJoinSquad) {
                    ...
                    await this.log(`Kamu sudah bergabung dengan squad ${squadTitle} | ${squadMembers} anggota`, 'info');
                }

                await this.randomDelay();
                const balance = await this.getAccountInfo(token);
                if (balance === null) {
                    await this.log('Balance: Tidak bisa membaca balance', 'error');
                    continue;
                } else {
                ...
                if (gameInfo === null) {
                    await this.log('Tidak bisa mengambil data game!', 'error');
                    continue;
                } else {
                    ...
                    await this.log(`Booster: Kotak ${specialBoxLeftRecoveryCount} | Recovery ${coinPoolLeftRecoveryCount}`, 'info');
                    await this.log(`Multivalue: Level ${singleCoinValue}`, 'info');
                    await this.log(`Batas Koin: Level ${singleCoinLevel}`, 'info');
                    await this.log(`Tingkat Isi: Level ${coinPoolRecoverySpeed}`, 'info');
                    await this.log(`Swipe Bot: Level ${swipeBotLevel}`, 'info');
                }

                await this.randomDelay();
                await this.log('Cek dan atur SwipeBot...', 'info');
                await this.handleSwipeBot(token);

                if (this.cekTaskEnable === 'y') {
                    await this.randomDelay();
                    await this.log('Mulai tugas...', 'info');
                    await this.processTasks(token);
                }

                if (this.upgradeMultiEnable === 'y') {
                    await this.randomDelay();
                    await this.log('Mulai upgrade multi...', 'info');
                    await this.upgradeLevel(token, gameInfo.data.singleCoinValue, this.maxLevel, '1');
                }

                if (this.upgradeFillEnable === 'y') {
                    await this.randomDelay();
                    await this.log('Mulai upgrade Tingkat Isi....', 'info');
                ...
                if (collectInfo === null) {
                    ...
                    await this.log(`Sisa energi ${coinPoolLeftCount}`, 'info');
                    ...
                    if (coinPoolLeftCount > 0) {
                        ...
                        if (collectResult && collectResult.code === 0) {
                            ...
                            await this.log(`Berhasil Tap, dapat ${collectedAmount} koin`, 'success');
                        } else {
                            await this.log('Tap gagal!', 'error');
                        }
                    }
                }

                await this.randomDelay();
                await this.log('Cek jumlah kotak yang tersisa...', 'info');
                if (gameInfo && gameInfo.data.specialBoxLeftRecoveryCount > 0) {
                    if (await this.useSpecialBox(token)) {
                        await this.randomDelay();
                        await this.log('Mulai mengumpulkan...', 'info');
                ...
                await this.randomDelay();
                await this.log('Mulai recovery...', 'info');
                ...
                if (updatedGameInfo && updatedGameInfo.data.coinPoolLeftRecoveryCount > 0) {
                    ...
                    await this.log('Recovery sudah habis!', 'warning');
                }

                await this.randomDelay();
                await this.log('Cek kotak gratis yang muncul...', 'info');
